...
{
	public partial class IPReturnAfterBilling : BasePage
	{
		...
		public static List<string> GetadvanceSearchIPNo(string prefixText, int count, string contextKey)
		{
			...
			getResult = ObjBO.GetadvanceSearchAfterBillingIPNo(ObjData);
		...
		protected void btnsave_Click(object sender, EventArgs e)
		{
			...
			try
			{
				...
				List<IPReturnData> result = objBO.UpdateIPReturnAfterBillingDetails(objdata);
				if (result.Count > 0)
				{
		...
		public List<IPReturnData> GetiPReturnList1(int curIndex)
		{
			...
			return objbo.GetiPReturnListAfterBill1(objdata);
		}
		protected void gvipreturnlist_RowCommand(object sender, GridViewCommandEventArgs e)
		{
			try
			{
				if (e.CommandName == "Print")
				{
					...
					string url = "../MedStore/Reports/ReportViewer.aspx?option=AfterBillingIPReturnList&ReturnNo=" + ReturnNo.Text.Trim();
		...
		protected void btnprint_Click(object sender, EventArgs e)
		{
			...
			string url = "../MedStore/Reports/ReportViewer.aspx?option=AfterBillingIPReturnList&ReturnNo=" + ReturnNo.Trim();
			string fullURL = "window.open('" + url + "', '_blank');";
			ScriptManager.RegisterStartupScript(this, typeof(string), "OPEN_New_Tab", fullURL, true);
		}
	}
