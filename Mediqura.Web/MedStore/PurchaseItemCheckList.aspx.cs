...
{
    public partial class PurchaseItemCheckList : BasePage
    {
        ...
        protected void bindgrid()
        {
            try
            {
                ...
                List<StockGRNData> objdeposit = GetItemCheckList(0);
        ...
        public List<StockGRNData> GetItemCheckList(int curIndex)
        {
        ...
        protected void btnaccept_Click(object sender, EventArgs e)
        {
            if (gvitemchecklist.Rows.Count == 0)
            {
                ...
                List<StockGRNData> SelectedItemList = Session["SelectedItemList"] == null ? new List<StockGRNData>() : (List<StockGRNData>)Session["SelectedItemList"];
                string ID;
                var source = txt_itemName.Text.ToString();
                if (source.Contains(":"))
                {
                ...
                if (SelectedItemList.Count > 0)
                {
                    ...
                    txt_itemName.Text = "";
                    Messagealert_.ShowMessage(lblmessage, "Succesfully added to the item checklist.", 1);
                    divmsg1.Visible = true;
                ...
                foreach (GridViewRow row in gvitemchecklist.Rows)
                {
                    ...
                    if (cb.Checked)
                    {
                        ...
                        foreach (GridViewRow row1 in gvitemselectedlist.Rows)
                        {
                            ...
                            {
                                Messagealert_.ShowMessage(lblmessage, "Successfully added to the item checklist.", 0);
                                divmsg1.Visible = true;
                                divmsg1.Attributes["class"] = "FailAlert";
                                return;
                            }
                    ...
                    if (SelectedItemList.Count > 0)
                    {
                        ...
                        gvitemselectedlist.Visible = true;
                        Session["SelectedItemList"] = SelectedItemList;
                        Messagealert_.ShowMessage(lblmessage, "Listcheck", 1);
                        divmsg1.Visible = true;
                        divmsg1.Attributes["class"] = "SucessAlert";
                        btn_send.Attributes.Remove("disabled");
                        txt_totusable.Text = "0";
        ...
        protected void bindgrid1()
        {
            try
            {
                ...
                List<StockGRNData> objdeposit = GetItemSelectedList(0);
                if (objdeposit.Count > 0)
                {
        ...
        public List<StockGRNData> GetItemSelectedList(int curIndex)
        {
            StockGRNData objstock = new StockGRNData();
            StockGRNBO objBO = new StockGRNBO();
        ...
        {
            List<StockGRNData> DepositDetails = GetItemSelectedList(0);
