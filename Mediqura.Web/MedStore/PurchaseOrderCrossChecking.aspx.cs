...
{
    public partial class PurchaseOrderCrossChecking : BasePage
    {
        ...
        private void bindddl()
        {
            ...
            Commonfunction.PopulateDdl(ddl_verifyby, mstlookup.GetLookupsList(LookupName.Employee));
        }
        ...
        {
            if (ddl_paymenttype.SelectedIndex == 2)
            {
                txt_dueamt.ReadOnly = false;
                txt_dueamt.Focus();
            }
            else
            {
                txt_dueamt.ReadOnly = true;
        ...
        protected void btn_verify_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                objrec.DueAmt = Convert.ToDecimal(txt_dueamt.Text == "" ? null : txt_dueamt.Text);
                if (ddl_paymenttype.SelectedIndex == 2)
                {
                    if (objrec.DueAmt >= (Convert.ToDecimal(txt_totalprice.Text.Trim() == "" ? "0" : txt_totalprice.Text.Trim())))
                    {
                        Messagealert_.ShowMessage(lblmessage, "Due amount should be less than total price.", 0);
                        divmsg1.Visible = true;
                        divmsg1.Attributes["class"] = "FailAlert";
                        txt_dueamt.Focus();
                        return;
                ...
                objrec.EmployeeID = Convert.ToInt64(ddl_verifyby.SelectedValue == "" ? "0" : ddl_verifyby.SelectedValue);
