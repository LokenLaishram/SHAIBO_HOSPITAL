...
{
	public partial class TransactionSummary : BasePage
	{
		...
		private void bindddl()
		{
			...
			Commonfunction.PopulateDdl(ddl_collectedby, mstlookup.GetLookupsList(LookupName.CollectionByPHRUser));
		...
		protected void btn_print_Click(object sender, EventArgs e)
		{
			...
			string CollectedByID = ddl_collectedby.SelectedValue == "" ? "0" : ddl_collectedby.SelectedValue;
			string AccountState = ddl_account_close.SelectedValue == "" ? "0" : ddl_account_close.SelectedValue;
			string AccountID = ddl_ledger.SelectedValue == "" ? "0" : ddl_ledger.SelectedValue;
			string timefrom = txttimepickerfrom.Text.Trim();
			string timeto = txttimepickerto.Text.Trim();

			string url = "../MedPhr/Reports/ReportViewer.aspx?option=TransactionStatement&TransactionID=" + TransactionID.Trim() + "&CollectedByID=" + CollectedByID + "&AccountState=" + AccountState + "&AccountID=" + AccountID + "&from=" + (from.ToString("yyyy-MM-dd") + " " + timefrom).ToString() + "&To=" + (To.ToString("yyyy-MM-dd") + " " + timeto).ToString();
		...
		protected void btn_Summary_Click(object sender, EventArgs e)
		{
			...
			string url = "../MedPhr/Reports/ReportViewer.aspx?option=TransactionSummary&from=" + (from.ToString("yyyy-MM-dd") + " " + timefrom).ToString() + "&To=" + (To.ToString("yyyy-MM-dd") + " " + timeto).ToString();
			string fullURL = "window.open('" + url + "', '_blank');";
			ScriptManager.RegisterStartupScript(this, typeof(string), "OPEN_New_Tab", fullURL, true);
		}
				...
				if (objincome.Count > 0)
				{
					...
					if (ddl_transaction.SelectedIndex == 0)
					{
						lbldescription.Text = " No. Of Income Transaction : " + objincome[0].noofIncometran.ToString()
										   ...
										   + " | No. Of Bank Income Transaction : " + objincome[0].noofincomebanktran.ToString()+Environment.NewLine
		...
		public List<TransactionSummaryData> GetIncomeTransactionList(int curIndex)
		{
			...
			ObjData.CollectedByID = Convert.ToInt64(ddl_collectedby.SelectedValue == "" ? "0" : ddl_collectedby.SelectedValue);
				...
				if (objexpenses.Count > 0)
				{
					...
					if (ddl_transaction.SelectedIndex == 0)
					{
						lbldescription.Text = " No. Of Income Transaction : " + objexpenses[0].noofIncometran.ToString()
										   ...
										   + " | No. Of Bank Income Transaction : " + objexpenses[0].noofincomebanktran.ToString() + Environment.NewLine
