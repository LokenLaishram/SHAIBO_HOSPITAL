...
namespace Mediqura.Web.MedHR
{
    ...
    public class EmployeeImageHandler : IHttpHandler
    {
        ...
        {
            string empImageID = context.Request.QueryString["empID"];
            SqlConnection con = new SqlConnection(GlobalConstant.ConnectionString);
            con.Open();
            SqlCommand cmd = new SqlCommand("SELECT EmployeePhoto FROM MDQ_EmployeeDetails WHERE EmployeeID=" + empImageID, con);
