...
{
    public partial class EmrgFinalBill : BasePage
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                {
                    txt_emrgno.Text = Session["BillPay"].ToString();
                    txt_emrgno.ReadOnly = true;
        ...
        public static List<string> GetEmrgNo(string prefixText, int count, string contextKey)
        {
            ...
            getResult = objInfoBO.GetPhrEmrgNo(Objpaic);
        ...
        {
            PHREmrgFinalData objData = new PHREmrgFinalData();
            EmrgAdmissionBO ObjBO = new EmrgAdmissionBO();
            List<PHREmrgFinalData> getResult = new List<PHREmrgFinalData>();
            objData.EmrgNo = txt_emrgno.Text.Trim() == "" ? "" : txt_emrgno.Text.Trim();
            objData.FinancialYearID = LogData.FinancialYearID;
            objData.HospitalID = LogData.HospitalID;
            getResult = ObjBO.Get_PHR_emrgfinalBilldetails(objData);
            ...
            try
            {
                ...
                if (e.CommandName == "Pay")
                {
                    ...
                    if (Result.Count > 0)
                    {
                        ...
                        txt_totalpaid.ReadOnly = true;
                        txt_emrgno.ReadOnly = true;
                        txt_emrgno.Text = Result[0].EmrgNo.ToString();
        ...
        protected void bindgridList()
        {
            try
            {
                ...
                List<PHREmrgFinalData> objdeposit = GetEMRGbillList(0);
        ...
        public List<PHREmrgFinalData> GetEMRGbillList(int curIndex)
        {
            PHREmrgFinalData objbill = new PHREmrgFinalData();
            ...
            return objbillingBO.GetPharamacyFinalBillList(objbill);
        }
            ...
            try
            {
                if (e.CommandName == "Deletes")
                {
                    if (LogData.RoleID == 1 || LogData.RoleID == 40)
                    {
                        ...
                        PHREmrgFinalData objadmin = new PHREmrgFinalData();
                        ...
                        int Result = obadminBO.DeletePHRemrgfinalbill(objadmin);
                        if (Result == 1)
                        {
                            bindgridList();
        ...
        protected void btnsave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                objData.EmrgNo = txt_emrgno.Text.Trim();
                ...
                List<PHREmrgFinalData> result = ObjBO.Update_PHR_EMRGFinal_BillDetails(objData);
                if (result.Count > 0)
                {
                    hdnpayable.Value = "";
