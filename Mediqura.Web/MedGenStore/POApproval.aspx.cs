     ...
     protected void Page_Load(object sender, EventArgs e)
        ...
        protected void bindgrid()
        {
            try
            {
                ...
                List<GENStrData> objdeposit = GetItemApprovalNoList(0);
        ...
        public List<GENStrData> GetItemApprovalNoList(int curIndex)
        {
            GENStrData objstock = new GENStrData();
            POApprovalBO objBO = new POApprovalBO();
        ...
        protected void gvitemtobeapprovedlist_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            try
            {
                if (e.CommandName == "Select")
                {
                    ...
                    POApprovalBO objstdBO = new POApprovalBO();
        ...
        protected void txt_approvedqty_TextChanged(object sender, EventArgs e)
        {
            ...
            TextBox qty = (TextBox)gvpurchaseorderlist.Rows[currentrow.RowIndex].Cells[0].FindControl("txt_approvedqty");
            TextBox tax = (TextBox)gvpurchaseorderlist.Rows[currentrow.RowIndex].Cells[0].FindControl("txt_tax");
            TextBox CPperQty = (TextBox)gvpurchaseorderlist.Rows[currentrow.RowIndex].Cells[0].FindControl("txt_cpperqty");
            Label CP = (Label)gvpurchaseorderlist.Rows[currentrow.RowIndex].Cells[0].FindControl("lbl_cp");
            CP.Text = ((Convert.ToDecimal(CPperQty.Text)) * (Convert.ToInt32(qty.Text))).ToString();
            CP.Text = (Convert.ToDouble(txt_totcp.Text) + (Convert.ToDouble(CP.Text) * Convert.ToDouble(tax.Text)) / 100).ToString();
            txt_totcp.Text = (Convert.ToDouble(txt_totcp.Text) + Convert.ToDouble(CP.Text)).ToString();
            foreach (GridViewRow row1 in gvpurchaseorderlist.Rows)
            {
                TextBox qty1 = (TextBox)gvpurchaseorderlist.Rows[row1.RowIndex].Cells[0].FindControl("txt_approvedqty");
                txt_totappreqd.Text = (Convert.ToInt32(txt_totappreqd.Text) + Convert.ToInt32(qty1.Text)).ToString();
                qty1.Focus();
            }
        ...
        protected void txt_cpperqty_TextChanged(object sender, EventArgs e)
        {
            ...
            foreach (GridViewRow row1 in gvpurchaseorderlist.Rows)
            {
                TextBox tax1 = (TextBox)gvpurchaseorderlist.Rows[currentrow.RowIndex].Cells[0].FindControl("txt_tax");
                TextBox qty1 = (TextBox)gvpurchaseorderlist.Rows[row1.RowIndex].Cells[0].FindControl("txt_approvedqty");
                txt_totappreqd.Text = (Convert.ToInt32(txt_totappreqd.Text) + Convert.ToInt32(qty1.Text)).ToString();
                CP.Text = (Convert.ToDouble(qty.Text) * Convert.ToDouble(CPperQty.Text) + (Convert.ToDouble(tax.Text) * Convert.ToDouble(qty.Text) * Convert.ToDouble(CPperQty.Text))/100).ToString();
                tax1.Focus();
            }
        }
        protected void btnreset_Click(object sender, EventArgs e)
        {
            ...
            gvitemtobeapprovedlist.DataSource = null;
            gvitemtobeapprovedlist.DataBind();
            gvitemtobeapprovedlist.Visible = false;
            ...
            try
            {
                ...
                foreach (GridViewRow row in gvpurchaseorderlist.Rows)
                {
                    ...
                    if (cb1.Checked)
                    {
                        ...
                        TextBox CPperQty = (TextBox)gvpurchaseorderlist.Rows[row.RowIndex].Cells[0].FindControl("txt_cpperqty");
                        ...
                        TextBox Tax = (TextBox)gvpurchaseorderlist.Rows[row.RowIndex].Cells[0].FindControl("txt_tax");
                ...
                List = objBO.UpdatePurchaseOrder(objrec);

                if (List.Count > 0)
                {
                    txt_purchaseOrderNo.Text = List[0].PONo.ToString();
        ...
        protected void bindgrid1()
        {
            try
            {
                ...
                List<GENStrData> objdeposit = GetPurchaseList(0);
                if (objdeposit.Count > 0)
                {
        ...
        public List<GENStrData> GetPurchaseList(int curIndex)
        {
            GENStrData objstock = new GENStrData();
            POApprovalBO objBO = new POApprovalBO();
        ...
        {
            List<GENStrData> DepositDetails = GetPurchaseList(0);
        ...
        protected void gvpurchaselist_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            try
            {
                if (e.CommandName == "Deletes")
                {
                    ...
                    int Result = objBO.DeletePurchaseLIstByID(objbill);
                    if (Result == 1)
                    {
        ...
        public void ExportToPdf()
        {
            using (StringWriter sw = new StringWriter())
            {
                using (HtmlTextWriter hw = new HtmlTextWriter(sw))
                {
                    ...
                    gvpurchaselist.Columns[5].Visible = false;
                    ...
                    Response.AddHeader("content-disposition", "attachment;filename=ItemApprovalList.pdf");
        ...
        protected void ExportoExcel()
        {
            ...
            using (XLWorkbook wb = new XLWorkbook())
            {
                ...
                Response.AddHeader("content-disposition", "attachment;filename=ItemApprovalList.xlsx");
