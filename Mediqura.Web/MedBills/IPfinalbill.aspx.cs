...
{
    public partial class IPfinalbill : BasePage
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                {
                    String ID = Session["IPNo"].ToString();
                    Session["IPNo"] = null;
        ...
        public static List<string> GetintimtedIPNos(string prefixText, int count, string contextKey)
        {
            ...
            getResult = objBO.GetIntimatedIPNos(objData);
        ...
        [System.Web.Services.WebMethodAttribute(), System.Web.Script.Services.ScriptMethodAttribute()]
        public static List<string> GetFIPno(string prefixText, int count, string contextKey)
        {
            IPData objData = new IPData();
            FInalBillBO objBO = new FInalBillBO();
            List<IPData> getResult = new List<IPData>();
            objData.IPNo = prefixText;
            getResult = objBO.GetFinalIPNo(objData);
            List<String> list = new List<String>();
            for (int i = 0; i < getResult.Count; i++)
            {
                list.Add(getResult[i].IPNo.ToString());
        ...
        protected void txt_ipnumber_TextChanged(object sender, EventArgs e)
        {
            Getfinalbilldetails("");
        }
        protected void Getfinalbilldetails(string emrgId)
        {
            btnaddresponsibility.Visible = false;
            txt_ipnumber.Text = emrgId == "" ? txt_ipnumber.Text : emrgId;
            Session["DiscountReqList"] = null;
            Session["DiscountList"] = null;
            Session["Responsibilty"] = null;
            IPfinalbillData objData = new IPfinalbillData();
            FInalBillBO objBO = new FInalBillBO();
            List<IPfinalbillData> getResult = new List<IPfinalbillData>();
            objData.IPNo = txt_ipnumber.Text.Trim() == "" ? "" : txt_ipnumber.Text.Trim();
            objData.FinancialYearID = LogData.FinancialYearID;
            objData.HospitalID = LogData.HospitalID;
            objData.AmountEnable = LogData.AmountEnable;
            getResult = objBO.GetIPfinalbillDetails(objData);
            if (getResult.Count > 0)
            {
                if (getResult[0].ChkPhrBillClear.ToString() == "1")
                {
                    ...
                    txt_ipnumber.Text = getResult[0].IPNo.ToString();
                    ...
                    if (Convert.ToDecimal(getResult[0].BalanceAmount.ToString()) <= 0)
                    {
                        ...
                        txt_totalpaid.Text = Commonfunction.Getrounding(getResult[0].TotalBillAmount.ToString());
                    ...
                    if (Convert.ToDecimal(getResult[0].BalanceAmount.ToString()) > 0 && Convert.ToDecimal(getResult[0].BalanceAmount.ToString()) < Convert.ToDecimal(getResult[0].TotalBillAmount.ToString()))
                    {
                        ...
                        txt_totalpaid.Text = Commonfunction.Getrounding((Convert.ToDecimal(getResult[0].TotalBillAmount.ToString()) - Convert.ToDecimal(getResult[0].BalanceAmount.ToString())).ToString());
                    ...
                    Gv_servicelist.DataSource = getResult;
                    Gv_servicelist.DataBind();
                    List<IPDiscountListData> DiscountReqList = Session["DiscountReqList"] == null ? new List<IPDiscountListData>() : (List<IPDiscountListData>)Session["DiscountReqList"];
                    ...
                    {
                        FInalBillBO objBOL = new FInalBillBO();
                        IPfinalbillData objdiscdata = new IPfinalbillData();
                        objdiscdata.IPNo = txt_ipnumber.Text.Trim();
                        List<IPDiscountListData> result = objBOL.GetDiscountListForIP(objData);
        ...
        int CategoryID = 0;
        int rowIndex = 1;
        protected void Gv_servicelist_RowCreated(object sender, GridViewRowEventArgs e)
        {
        ...
        protected void Gv_servicelist_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            try
            {
                if (e.CommandName == "Deletes")
                {
                    ...
                    IPfinalbillData objadmin = new IPfinalbillData();
                    FInalBillBO obadminBO = new FInalBillBO();
                    int i = Convert.ToInt16(e.CommandArgument.ToString());
                    GridViewRow gr = Gv_servicelist.Rows[i];
                    ...
                    objadmin.IPNo = txt_ipnumber.Text == "" ? "" : txt_ipnumber.Text;
                    ...
                    int Result = obadminBO.DeleteIPservices(objadmin);
                    if (Result == 1)
                    {
                        Getfinalbilldetails("");
        ...
        protected void Gvfinalbillist_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            try
            {
                if (e.CommandName == "Deletes")
                {
                    ...
                    FInalBillBO obadminBO = new FInalBillBO();
                    int i = Convert.ToInt16(e.CommandArgument.ToString());
                    GridViewRow gr = Gvfinalbillist.Rows[i];
                    Label Bill = (Label)gr.Cells[0].FindControl("lbl_billno");
                    Label ipnumber = (Label)gr.Cells[0].FindControl("lbl_ipnumber");
                ...
                if (e.CommandName == "Print")
                {
                    ...
                    GridViewRow gp = Gvfinalbillist.Rows[j];
                    Label billno = (Label)gp.Cells[0].FindControl("lbl_billno");
                    string url = "../MedBills/Reports/ReportViewer.aspx?option=FinalBill&BillNo=" + billno.Text.ToString();
        ...
        protected void btnsave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                IPfinalbillData objbill = new IPfinalbillData();
                List<Discount> ListDiscount = new List<Discount>();
                FInalBillBO objstdBO = new FInalBillBO();
                ...
                objbill.IPNo = txt_ipnumber.Text.Trim();
                ...
                List<IPfinalbillData> result = objstdBO.UpdatFinalBill(objbill);
                if (result.Count > 0)
                {
                    lbl_bill.InnerText = "Bill Number";
        ...
        {
            txt_finalipnumber.Text = "";
            txtdatefrom.Text = "";
            txtto.Text = "";
            ddlstatus.SelectedIndex = 0;
            Gvfinalbillist.DataSource = null;
            Gvfinalbillist.DataBind();
            Gvfinalbillist.Visible = false;
            ...
            btnDisReq.Attributes.Remove("disabled");
        }
        ...
        {
            List<IPfinalbillData> ServiceDetails = GetBilllist(0);
            List<IpbillDataToExcel> ListexcelData = new List<IpbillDataToExcel>();
            int i = 0;
            foreach (IPfinalbillData row in ServiceDetails)
            {
                IpbillDataToExcel Ecxeclpat = new IpbillDataToExcel();
        ...
        public void ExportToPdf()
        {
            using (StringWriter sw = new StringWriter())
            {
                ...
                {
                    Gvfinalbillist.BorderStyle = BorderStyle.None;
                    //Hide the Column containing CheckBox
                    Gvfinalbillist.Columns[12].Visible = false;
                    Gvfinalbillist.Columns[13].Visible = false;
                    Gvfinalbillist.RenderControl(hw);
                    Gvfinalbillist.HeaderRow.Style.Add("width", "15%");
                    Gvfinalbillist.HeaderRow.Style.Add("font-size", "10px");
                    Gvfinalbillist.Style.Add("text-decoration", "none");
                    Gvfinalbillist.Style.Add("font-family", "Arial, Helvetica, sans-serif;");
                    Gvfinalbillist.Style.Add("font-size", "8px");
        ...
        protected void bindgridList()
        {
            try
            {
                ...
                List<IPfinalbillData> objdeposit = GetBilllist(0);
                if (objdeposit.Count > 0)
                {
                    Gvfinalbillist.DataSource = objdeposit;
                    Gvfinalbillist.DataBind();
                    Gvfinalbillist.Visible = true;
        ...
        public List<IPfinalbillData> GetBilllist(int curIndex)
        {
            IPfinalbillData objbill = new IPfinalbillData();
            ...
            string UHID;
            string source = txtpatientNames.Text.ToString();
            if (source.Contains(":"))
            {
                UHID = source.Substring(source.LastIndexOf(':') + 1);
                objbill.UHID = Convert.ToInt64(UHID.Trim());
            }
            else
            {
                objbill.UHID = 0;
            ...
            objbill.IPNo = txt_finalipnumber.Text.Trim() == "" ? "" : txt_finalipnumber.Text.Trim();
            objbill.PatientName = txtpatientNames.Text == "" ? "" : txtpatientNames.Text.Trim();
            ...
            return objbillingBO.GetIPDfinalbillList(objbill);
        }
        ...
        {
            string url = "../MedBills/Reports/ReportViewer.aspx?option=FinalBill&BillNo=" + hdnbillnumber.Value.ToString();
            string fullURL = "window.open('" + url + "', '_blank');";
            ScriptManager.RegisterStartupScript(this, typeof(string), "OPEN_New_Tab", fullURL, true);
        }
        ...
        protected void btn_refund_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    btn_refund.Attributes["disabled"] = "disabled";
                    //lbl_bill.InnerText = "Refund Number";
                    //txt_billnumber.Text = Listbill[0].RefundNo.ToString();
        ...
        protected void btnlinkdiscount_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                FInalBillBO objBO = new FInalBillBO();
                IPfinalbillData objData = new IPfinalbillData();
                objData.IPNo = txt_ipnumber.Text;
                List<IPDiscountListData> result = objBO.GetDiscountListForIP(objData);
                GVDiscountReq.DataSource = result;
                GVDiscountReq.DataBind();
                GVDiscountReq.Visible = true;
                txtDisTotalBillAmount.Text = result[0].TotalBill.ToString();
        ...
        {
            decimal totalDiscount = 0;
            List<IPDiscountListData> DiscountReqList = new List<IPDiscountListData>();
            foreach (GridViewRow row in GVDiscountReq.Rows)
            {
                IPDiscountListData objData = new IPDiscountListData();
        ...
        protected void btnDisReq_Click(object sender, EventArgs e)
        {
            ...
            objData.XMLData = XmlConvertor.IpAdmissionDiscountToXML(DiscountReqList).ToString();
            objData.IPNo = txt_ipnumber.Text;
            ...
            if (result[0].Resultoutput > 0)
            {
                ...
                {
                    btnDisReq.Attributes["disabled"] = "disabled";
